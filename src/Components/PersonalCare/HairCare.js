...
const HairCare = () => {
    ...
    const getHairCare = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/personalCare/hairCare/get", {
                headers: {
    ...
    const searchHairCare = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/personalCare/hairCare/searchHairCare/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
