...
const MenGrooming = () => {
    ...
    const getGrooming = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/personalCare/menGrooming/get", {
                headers: {
    ...
    const searchMenGrooming = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/personalCare/menGrooming/searchMenGrooming/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
