...
const Fragrances = () => {
    ...
    const getFragrance = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/personalCare/fragrances/get", {
                headers: {
    ...
    const searchFragrance = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/personalCare/fragrances/searchFragrance/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
