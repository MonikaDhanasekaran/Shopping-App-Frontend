...
const OfficeChairs = () => {
    ...
    const getOfficeChair = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/furniture/officeChairs/get", {
                headers: {
    ...
    const searchOfficeChairs = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/furniture/officeChairs/searchOfficeChairs/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
