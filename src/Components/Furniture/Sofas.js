...
const Sofas = () => {
    ...
    const getSofa = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/furniture/sofas/get", {
                headers: {
    ...
    const searchSofas = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/furniture/sofas/searchSofas/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
