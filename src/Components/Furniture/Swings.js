...
const Swings = () => {
    ...
    const getSwing = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/furniture/swings/get", {
                headers: {
    ...
    const searchSwings = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/furniture/swings/searchSwings/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
