...
const Jean = () => {
    ...
    const getJean = async () => {
        try {
            const response = await axios.get("https://shopping-app-xkoy.onrender.com/shop/fashion/jeans/get", {
                headers: {
    ...
    const searchJeans = async (event) => {
        ...
        if (key) {
            let result = await fetch(`https://shopping-app-xkoy.onrender.com/shop/fashion/jeans/searchJeans/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
