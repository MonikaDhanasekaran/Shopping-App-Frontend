...
const TrackPants = () => {
    ...
    const getPant = async () => {
        try {
            const response = await axios.get("https://shopping-app-xkoy.onrender.com/shop/fashion/trackPants/get", {
                headers: {
    ...
    const searchTrackpants = async (event) => {
        ...
        if (key) {
            let result = await fetch(`https://shopping-app-xkoy.onrender.com/shop/fashion/trackPants/searchTrackpants/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
