...
const LuggagesAndBags = () => {
    ...
    const getBag = async () => {
        try {
            const response = await axios.get("https://shopping-app-xkoy.onrender.com/shop/fashion/luggageAndBags/get", {
                headers: {
    ...
    const searchLuggagesAndBags = async (event) => {
        ...
        if (key) {
            let result = await fetch(`https://shopping-app-xkoy.onrender.com/shop/fashion/luggageAndBags/searchLuggagesAndBags/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
