...
const WomenFootwear = () => {
    ...
    const getFootwear = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/fashion/womenFootwear/get", {
                headers: {
    ...
    const searchWomenFootwear = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/fashion/womenFootwear/searchWomenFootwear/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
