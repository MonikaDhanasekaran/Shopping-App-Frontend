...
const Speakers = () => {
    ...
    const getSpeaker = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/electronics/speakers/get", {
                headers: {
    ...
    const searchSpeakers = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/electronics/speakers/searchSpeakers/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
