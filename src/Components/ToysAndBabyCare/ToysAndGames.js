...
const ToysAndGames = () => {
    ...
    const getToy = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/toysAndBabyCare/toysAndGames/get", {
                headers: {
    ...
    const searchToysAndGames = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/toysAndBabyCare/toysAndGames/searchToysAndGames/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
