...
const PopularItem = () => {
    ...
    const getPopularItem = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/popularItem/get", {
                headers: {
    ...
    const handlePopularItemSearch = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/popularItem/searchPopularItems/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
