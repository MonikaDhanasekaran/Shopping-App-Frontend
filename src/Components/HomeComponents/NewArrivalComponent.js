...
const NewArrival = () => {
    ...
    const getNewArrival = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/newArrival/get", {
                headers: {
    ...
    const handleNewArrivalSearch = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/newArrival/searchNewArrivals/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
