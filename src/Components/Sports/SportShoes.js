...
const SportShoes = () => {
    ...
    const getShoe = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/sports/sportShoes/get", {
                headers: {
    ...
    const searchSportShoes = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/sports/sportShoes/searchSportShoes/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
