...
const FitnessEquipments = () => {
    ...
    const getFitness = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/sports/fitnessEquipments/get", {
                headers: {
    ...
    const searchFitnessEquipments = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/sports/fitnessEquipments/searchFitnessEquipments/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
