...
const GardeningEssentials = () => {
    ...
    const getGardening = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/home/gardeningEssentials/get", {
                headers: {
    ...
    const searchGardeningEssentials = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/home/gardeningEssentials/searchGardeningEssentials/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
