...
const KitchenAndCookware = () => {
    ...
    const getCookware = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/home/kitchenAndCookware/get", {
                headers: {
    ...
    const searchKitchenAndCookware = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/home/kitchenAndCookware/searchKitchenAndCookware/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
