...
const Refrigerators = () => {
    ...
    const getFridge = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/appliances/refrigerator/get", {
                headers: {
    ...
    const searchRefrigerator = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/appliances/refrigerator/searchRefrigerator/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
