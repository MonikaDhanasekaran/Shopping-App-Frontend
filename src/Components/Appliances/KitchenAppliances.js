...
const KitchenAppliances = () => {
    ...
    const getKitchen = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/appliances/kitchenAppliances/get", {
                headers: {
    ...
    const searchKitchenAppliances = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/appliances/kitchenAppliances/searchKitchenAppliances/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
