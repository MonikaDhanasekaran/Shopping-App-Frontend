...
const AirConditioners = () => {
    ...
    const getAC = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/appliances/airConditioners/get", {
                headers: {
    ...
    const searchAirConditioners = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/appliances/airConditioners/searchAirConditioners/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
