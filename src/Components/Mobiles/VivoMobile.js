...
const VivoMobile = () => {
    ...
    const getVivo = async () => {
        try {
            const response = await axios.get("https://shopping-app-xkoy.onrender.com/shop/mobile/vivo/get", {
                headers: {
    ...
    const searchVivoMobile = async (event) => {
        ...
        if (key) {
            let result = await fetch(`https://shopping-app-xkoy.onrender.com/shop/mobile/vivo/searchVivo/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
