...
const GooglePixelMobile = () => {
    ...
    const getGooglePixel = async () => {
        try {
            const response = await axios.get("http://localhost:5000/shop/mobile/googlePixel/get", {
                headers: {
    ...
    const searchGooglePixelMobile = async (event) => {
        ...
        if (key) {
            let result = await fetch(`http://localhost:5000/shop/mobile/googlePixel/searchGooglePixel/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
