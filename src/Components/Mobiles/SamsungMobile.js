...
const SamsungMobile = () => {
    ...
    const getSamsung = async () => {
        try {
            const response = await axios.get("https://shopping-app-xkoy.onrender.com/shop/mobile/samsung/get", {
                headers: {
    ...
    const searchSamsungMobile = async (event) => {
        ...
        if (key) {
            let result = await fetch(`https://shopping-app-xkoy.onrender.com/shop/mobile/samsung/searchSamsung/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
