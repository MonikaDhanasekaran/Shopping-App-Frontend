...
const PocoMobile = () => {
    ...
    const getPoco = async () => {
        try {
            const response = await axios.get("https://shopping-app-xkoy.onrender.com/shop/mobile/poco/get", {
                headers: {
    ...
    const searchPocoMobile = async (event) => {
        ...
        if (key) {
            let result = await fetch(`https://shopping-app-xkoy.onrender.com/shop/mobile/poco/searchPoco/${key}`, {
                headers: {
                    accesstoken: localStorage.getItem("token"),
                }
            });
